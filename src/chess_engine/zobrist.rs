...
impl Zobrist {
    pub fn new() -> Self {
        ...
        for hash in table.iter_mut().take(ZOBRIST_TABLE_LENGTH) {
            *hash = rng.random::<u32>().into();
        }

        Self { table }
    }

    // pub fn gen_initial_hash_mailbox(&self, board: &[Option<LegacyPiece>]) -> ZobristHash {
    //     let mut hash = 0.into();
...
mod tests {
    ...
    use crate::chess_engine::{Game, pieces::WHITE_KNIGHT};
    ...
    fn hash_on_default() {
        ...
        assert_ne!(board.boards.zobrist_hash, 0.into());
    }
    ...
    fn hash_same_seed() {
        ...
        assert_eq!(board.boards.zobrist_hash, other_board.boards.zobrist_hash);
    }
